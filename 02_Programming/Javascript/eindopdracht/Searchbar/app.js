...
function filter (ev) {
    ...
    if(result.length > 0 ) {                                              // als resultaat van vergl gevonden dan geef voor ieder
      ...
      temp =`<li class='search-item'></li>`;                            // anders geef leeg list item
    }
    output.innerHTML = temp;                                            // output html 
}

input.addEventListener("input", function(event){                        // nog niet werkende event
    if (input.value.length = 0 ) {                                      // listener om zoekwoorden te 
      function changeStyle(){                                           // verbergen bij lege searchbar
        const element = document.getElementsByClassName("output");
...
// - list items verbergen als er geen input in input is.
//
//
